...
class RunnerBase:
    ...
    def device(self):
        ...
        # print("showing self device: ", self._device)
        return self._device
        ...
        if self._model.device != self.device:
            ...
            if self.use_distributed:
                if self._wrapped_model is None:
                    # print("gpu:    ", self.config.run_cfg.gpu)
                    self._wrapped_model = DDP(
                        self._model, device_ids=[self.config.run_cfg.gpu],
                        find_unused_parameters=True
                    )
                    # self._wrapped_model = DDP(
                    #     self._model, device_ids=[6, 7],
                    #     find_unused_parameters=True
                    # )
                    # print("pass")
            else:
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        bin_save_to = os.path.join(
            self.output_dir,
            "checkpoint_{}.bin".format("best" if is_best else cur_epoch),
        )
        logging.info("Saving checkpoint at epoch {} to {}.".format(cur_epoch, save_to))
        torch.save(save_obj, save_to)
        checkpoint = torch.load(save_to, map_location='cpu') 
        state_dict = checkpoint["model"] 
        # torch.save(state_dict, bin_save_to)
